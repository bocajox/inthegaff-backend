    ...
    try {
      ...
      for (const card of cards) {
        try {
          ...
          // Address — "Studio Flat, John Dalton St, M2"
          // The address is typically after the beds/type info
          ...
          let street = '';
          if (addrMatch) {
            street = addrMatch[1].replace(/,\s*$/, '').trim();
          ...
          let photo = '';
          if (img.length) {
            photo = img.attr('src') || img.attr('data-src') || '';
            // OpenRent uses lazy loading — check for real image URL
          ...
          // Description snippet
          const descMatch = text.match(/(?:💰.*?|📍.*?)?(.{20,200})$/);
          const description = descMatch ? descMatch[1].trim() : '';

          results.push({
            externalId,
            title: '',  // auto-generated by scheduler
            price,
            beds,
            type,
            street,
