...
module.exports = {
  ...
  async scrape() {
    ...
    while (true) {
      ...
      let cards = $('ul.properties li, .propertyhive-property, a.card').toArray();
      if (!cards.length) {
        cards = $('article').filter((i, el) => {
          const t = $(el).text();
          return t.includes('£') || t.includes('pcm') || t.includes('PCM');
        }).toArray();
      }
      if (!cards.length) {
        cards = $('[class*="property-card"], [class*="property_card"], .property, .listing-item').toArray();
      }
      ...
      for (const card of cards) {
        ...
        const priceStr = el.find('.price, [class*="price"], [class*="Price"]').first().text().trim()
      ...
      const hasNext = $('a[rel="next"], .propertyhive-pagination .next, .next-page, [aria-label="Next"]').length > 0;
