...
module.exports = {
  id: 'gascoignehalman', name: 'Gascoigne Halman', website: BASE,
  areas: ['Didsbury', 'Chorlton', 'Sale', 'Altrincham', 'Wilmslow'],
  async scrape() {
    const listings = [];
    let page = 1;
    while (true) {
      const $ = await fetchHTML(`${BASE}/properties/to-rent/?page=${page}`);
      const cards = $('[class*="PropertyCard"], .property-card, [class*="listing"]').toArray();
      ...
      if (!$('a[rel="next"], .next').length || page >= 15) break;
