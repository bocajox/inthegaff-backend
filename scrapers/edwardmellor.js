...
module.exports = {
  id: 'edwardmellor', name: 'Edward Mellor', website: BASE,
  areas: ['Levenshulme', 'Gorton', 'Stockport', 'Fallowfield'],
  async scrape() {
    const listings = [];
    let page = 1;
    while (true) {
      const $ = await fetchHTML(`${BASE}/property/lettings/?page=${page}`);
      const cards = $('[class*="property-card"], .property-item, .listing').toArray();
