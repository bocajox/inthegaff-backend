...
module.exports = {
  id: 'belvoir', name: 'Belvoir', website: BASE,
  areas: ['Manchester City Centre', 'Salford', 'Chorlton', 'Didsbury'],
  async scrape() {
    const listings = [];
    let page = 1;
    while (true) {
      const $ = await fetchHTML(`${BASE}/manchester/properties/to-rent/?page=${page}`);
      const cards = $('[class*="property-card"], .property-card, [class*="listing"]').toArray();
