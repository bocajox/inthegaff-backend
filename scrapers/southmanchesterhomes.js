...
module.exports = {
  id: 'southmanchesterhomes', name: 'South Manchester Homes', website: BASE,
  areas: ['Chorlton', 'Didsbury', 'Withington', 'Whalley Range'],
  async scrape() {
    const listings = [];
    let page = 1;
    while (true) {
      const $ = await fetchHTML(`${BASE}/properties/?page=${page}`);
      const cards = $('[class*="property"], .property-item, .listing-item').toArray();
